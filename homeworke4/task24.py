...
# за один заход, находясь непосредственно перед некоторым кустом, собирает ягоды с этого куста и с двух соседних с ним. Напишите
# программу для нахождения максимального числа ягод, которое может собрать за один заход собирающий модуль, находясь перед некоторым
# кустом заданной во входном файле грядки.


from random import randint
num = int(input('Введите количество кустов ягод: '))
row = [randint(0, 20) for i in range(num)]
print(row)
max_position = 0
max_sum = 0
...
print(f" Индекс куста, находясь перед которым, модуль собирает максимальное количество ягод: {max_position}. Максимальное\
    количество собранных ягод = {max_sum} ")